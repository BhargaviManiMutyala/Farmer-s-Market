...
export default function BuyerRequests() {
  ...
  const handleCancelRequest = async (requestId) => {
    try {
      // Send a request to cancel the buyer's request
      await axios.delete(`http://localhost:5000/api/requests/${requestId}`);
      setRequests(prev => prev.filter(request => request._id !== requestId)); // Remove from UI
          ...
          {requests.length > 0 ? (
            requests.map((request) => (
              <div className="card" key={request._id}>
                ...
                <button onClick={() => handleCancelRequest(request._id)}>Cancel Request</button>
              </div>
            ))
          ) : (
